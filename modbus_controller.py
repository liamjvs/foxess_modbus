...
_F_G3_SERIAL_LENGTH = 8  # enough to read typical 15-character alphanumeric serials
# Synthesised model string for FoxESS Smart GW-M1 fallback detection.
...
class ModbusController(EntityController, UnloadController):
    ...
    async def autodetect(client: ModbusClient, slave: int, adapter_config: dict[str, Any]) -> tuple[str, str]:
        ...
        try:
            ...
            # Fallback: FoxESS Smart GW-M1 WiFi dongle.
            # The dongle does not expose registers 30000–38999 over Modbus TCP; instead it
            # stores a packed-ASCII serial number at holding registers 39018–39025.
            # If we can read a valid serial there we synthesise the F-G3 model string.
            if full_model is None:
                try:
                    ...
                    _LOGGER.debug("Autodetect: Smart GW-M1 fallback also failed for %s: %s", client, fallback_ex)
